...
document.getElementById("contactForm").addEventListener("submit", function (e) {
  ...
  emailjs.init("ZlOFV8-STX39OOSJI"); // Replace with your actual EmailJS Public Key

  const templateParams = {
    name: document.getElementById("name").value,  // Matches {{name}}
    email: document.getElementById("email").value, // Matches {{email}}
    message: document.getElementById("message").value, // Matches {{message}}
  };

  emailjs.send("service_30wpivz", "template_hsv83hw", templateParams)
    ...
    .catch(function (error) {
      ...
      alert("❌ Error sending message: " + JSON.stringify(error, null, 2));
    });
});
